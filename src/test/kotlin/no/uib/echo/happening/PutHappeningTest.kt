...
class PutHappeningTest : StringSpec({
    ...
    "When trying to submit a happening, server should respond with OK." {
        for (t in be) {
            val testCall = client.put(routeWithSlug(exHap(t).slug)) {
    ...
    "Whe trying to update happening spots, server should respond with OK." {
        for (t in be) {
            val submitHappeningCall = client.put(routeWithSlug(exHap(t).slug)) {
                contentType(ContentType.Application.Json)
                setBody(Json.encodeToString(removeSlug(exHap(t))))
            }
            ...
            val updateHappeningCall = client.put(routeWithSlug(exHap(t).slug)) {
                contentType(ContentType.Application.Json)
    ...
    "When trying to update a happening with the exact same values, server should respond with ACCEPTED." {
        for (t in be) {
            val submitBedpresCall = client.put(routeWithSlug(exHap(t).slug)) {
                contentType(ContentType.Application.Json)
                basicAuth("admin", System.getenv("ADMIN_KEY"))
                setBody(Json.encodeToString(removeSlug(exHap(t))))
            }
            ...
            val updateBedpresCall = client.put(routeWithSlug(exHap(t).slug)) {
                contentType(ContentType.Application.Json)
                basicAuth("admin", System.getenv("ADMIN_KEY"))
                setBody(Json.encodeToString(removeSlug(exHap(t))))
            }
